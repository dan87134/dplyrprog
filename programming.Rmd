...
<div class=r-comment>
<p>Comments are put inside of div tags like so:
<pre>
&lt;div class=r-comment&gt;
a comment
&lt;/div&gt;
...
<div class=r-comment>
<p>General comment
<p>The thing I have found about writing papers/courses is that it is hard to put yourself in a frame of mind of the read who know much less about the topic than you.
...
<div class=r-comment>
<p>"quoting function" is not a standard R term, is it? If a reader looked for R quoting functions they would find no details about them. You should make that clear to reader. It would be better to use a standard R term for this but I don't think there is one.

<p>Isn't a dplyr verb the same as a dplyr function? For the reader to understand what you are talking about they would have to understand grammars and have read the doc's about how dplyr is a grammer for data access/manipulation. That's a lot to ask and it really isn't a level of knowledge need to make use of dplyr.

<p>But since verb is standard R term that would be better to use than quoting function.

<p>BTW when I see the term "quoting functions" I thing of quote(), rlang::quo and so on. So as is I think quoting functions is a confusing description.
</div>
...
select(t, size = a)
```
You can pick off columns from a data.frame without having to use filters or even quotation marks.
...
The thing that distinguishes verbs from regular functions is in the way verbs treat their arguments. Unforunately R documentation doesn't tell you if a function is a verb or regular function. That distinction doesn't matter except when you are using them to do dplyr programming.



We will call these **quoting functions**. That's not a standard term and the documentation for R doesn't tell you if a function is a standard or quoting function. But quoting functions are what make "programming on the language" possible so we need some way to refer to them. 

In this vignette you will learn about quoting functions, what challenges they pose for
